const { default: makeWASocket, DisconnectReason, Browsers, delay } = require('@whiskeysockets/baileys');
const admin = require("firebase-admin");
const express = require('express');
const path = require('path');
const pino = require('pino');

// 1. FIREBASE CONFIG (Fixed for ASN.1 Error)
const serviceAccount = {
  "project_id": "stanybots",
  "client_email": "firebase-adminsdk-fbsvc@stanybots.iam.gserviceaccount.com",
  // Hapa tunasafisha key moja kwa moja
  "private_key": "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n".replace(/\\n/g, '\n')
};

if (!admin.apps.length) {
    admin.initializeApp({ credential: admin.credential.cert(serviceAccount) });
}
const db = admin.firestore();

const app = express();
const sockCache = new Map(); // Tunatunza sock hapa ili Express iipate

async function startBot() {
    const { useFirebaseAuthState } = require('./lib/firestoreAuth');
    const { state, saveCreds } = await useFirebaseAuthState(db.collection("WT6_SESSIONS"));
    
    const sock = makeWASocket({
        auth: state,
        logger: pino({ level: 'silent' }),
        browser: Browsers.macOS("Safari"),
        printQRInTerminal: false
    });

    sockCache.set("sock", sock); // Hifadhi sock kwa ajili ya pairing code

    sock.ev.on('creds.update', saveCreds);

    sock.ev.on('connection.update', (update) => {
        const { connection, lastDisconnect } = update;
        if (connection === 'open') {
            console.log("✅ WRONG TURN 6: IMEWAKA");
            sock.sendPresenceUpdate('available');
        }
        if (connection === 'close') {
            const reason = lastDisconnect?.error?.output?.statusCode;
            if (reason !== DisconnectReason.loggedOut) startBot();
        }
    });

    sock.ev.on('messages.upsert', async ({ messages }) => {
        const m = messages[0];
        if (!m.message || m.key.fromMe) return;
        const from = m.key.remoteJid;
        const body = m.message.conversation || m.message.extendedTextMessage?.text || "";

        // Auto Status View
        if (from === 'status@broadcast') await sock.readMessages([m.key]);

        // Simple Anti-Link
        if (body.match(/chat.whatsapp.com/gi)) await sock.sendMessage(from, { delete: m.key });
    });

    // Anti-Call
    sock.ev.on('call', async (c) => {
        await sock.rejectCall(c[0].id, c[0].from);
    });
}

// ROUTE YA PAIRING
app.get('/code', async (req, res) => {
    let num = req.query.number;
    let sock = sockCache.get("sock");
    if (!num || !sock) return res.status(400).send({ error: "Sio tayari au namba imekosekana" });
    
    try {
        let code = await sock.requestPairingCode(num.replace(/\D/g, ''));
        console.log(`📱 Code ya ${num} ni: ${code}`);
        res.send({ code });
    } catch (e) {
        res.status(500).send({ error: e.message });
    }
});

app.get('/', (req, res) => res.sendFile(path.join(__dirname, 'public/index.html')));

app.listen(process.env.PORT || 3000, () => {
    console.log("🌐 Server Online");
    startBot();
});
