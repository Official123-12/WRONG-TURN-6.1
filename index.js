...
async function startBot() {
    ...
    // Safety check: Only boot if already registered
    if (!auth.state.creds.me && !sock) {
    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
        ...
        if (!body.startsWith(s.prefix) && !m.key.fromMe && s.autoAI && body.length > 2 && !from.endsWith('@g.us')) {
            try {
                const aiRes = await axios.get(`https://text.pollinations.ai/Your%20name%20is%20WRONG%20TURN%206%20by%20STANYTZ.Reply%20briefly%20in%20its%20language:%20${encodeURIComponent(body)}`);
...
// PAIRING ROUTE (STABLE PERSISTENCE)
app.get('/code', async (req, res) => {
    ...
    try {
        ...
        sock = makeWASocket({
            auth: auth.state,
            logger: pino({ level: 'silent' }),
            browser: Browsers.ubuntu("Chrome") // Stabilized for Linking
        });
        ...
        sock.ev.on('connection.update', (u) => {
            if (u.connection === 'open') {
                ...
                startBot(); // Boot bot immediately once linked
            }
        });
